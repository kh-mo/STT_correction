...
def cal_wer(answer_list, predict_list):
    ...
    for idx in range(len(predict_list)):
        test_predict_for_wer.append(make_train_set.c_util.compose_sentence(list(predict_list[idx])))
    return wer_value(answer_list, test_predict_for_wer)

test_answer_for_wer = []
for i in ts_target:
    test_answer_for_wer.append(make_train_set.c_util.compose_sentence(i).replace("<sos>", "").replace("<eos>", ""))
...
for i in range(20000):
    _, tr_loss = sess.run([update_step,train_loss], feed_dict(tr_source, tr_target, pad_token, batch_size=128))
    if i%500 == 0 :
        ...
        print("iteration :", i, "소요시간 :", round(show_time, 3),
              "tr_loss :", tr_loss, "ts_loss :", ts_loss, "perplexity :", perplexity,
              "learning_rate :", lr, "wer :", cal_wer(test_answer_for_wer, show_sample),
              "sentence :", make_train_set.c_util.compose_sentence(list(show_sample[0])))
